...
from RL_Slover import SarsaNStep
import numpy as np
...
if __name__ == "__main__":
    ...
    for env_index in range(num_test):        
        epsilon, alpha, gamma, n_step = 0.1, 0.1, 0.9, 5
        env_type = envs.env_fns[0]()
        agents.append(SarsaNStep(env_type, n_step, alpha, gamma, epsilon, seeds[env_index])) 
        ...
        for i, agent in enumerate(agents):
            ...
            if not dones[i]:
                if terminateds[i] or truncateds[i]:
                    ...
                    agent.update_Q_table(observations[i], actions[i], rewards[i], infos['final_observation'][i], agent.take_action(infos['final_observation'][i]), done=(terminateds[i] or truncateds[i]))  
                else:
                    agent.update_Q_table(observations[i], actions[i], rewards[i], next_observations[i], agent.take_action(next_observations[i]), done=(terminateds[i] or truncateds[i]))
    ...
    plt.title('Sarsa-5Step on {}'.format('Cliff Walking'))
