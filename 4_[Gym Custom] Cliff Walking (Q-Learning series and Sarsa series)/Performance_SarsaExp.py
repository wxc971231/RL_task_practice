...
if __name__ == "__main__":
        ...
        for i, agent in enumerate(agents):
            ...
            if not dones[i]:
                if terminateds[i] or truncateds[i]:
                    ...
                    agent.update_Q_table(observations[i], actions[i], rewards[i], infos['final_observation'][i])  
                else:
                    agent.update_Q_table(observations[i], actions[i], rewards[i], next_observations[i])
    ...
    # 绘制 return 变化图
    np.save('data/SarsaExp', envs_returns)
