...
alpha = 0.1
gamma = 0.9
agent = QLearningDouble(wrapped_env, alpha, gamma, epsilon, seed=42)

# 进行训练
num_episodes = 1400         # 训练交互轨迹总量
    ...
    with tqdm(total=int(num_episodes / num_period), desc='Iteration %d' % i) as pbar:   # tqdm的进度条功能
        for i_episode in range(int(num_episodes / num_period)):                         # 每个进度条的序列数
            ...
            while True:
                ...
                agent.update_Q_table(observation, action, reward, next_observation, batch_size=0)
                agent.update_policy()
