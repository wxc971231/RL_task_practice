...
if __name__ == "__main__":
    ...
    for env_index in range(num_test):        
        ...
        agents.append(NStepTreeBackup(env_type, n_step, alpha, gamma, epsilon, seeds[env_index])) 
        ...
        for i, agent in enumerate(agents):
            ...
            if not dones[i]:
                if terminateds[i] or truncateds[i]:
                    ...
                    agent.update_Q_table(observations[i], actions[i], rewards[i], infos['final_observation'][i], done=(terminateds[i] or truncateds[i]))  
                else:
                    agent.update_Q_table(observations[i], actions[i], rewards[i], next_observations[i], done=(terminateds[i] or truncateds[i]))
    ...
    # 绘制 return 变化图
    np.save('data/5StepTreeBackup', envs_returns)

    plt.xlabel('Episodes')
    plt.ylabel('Ave Returns')
    plt.title('5 Step Tree Backup on {}'.format('Cliff Walking'))
