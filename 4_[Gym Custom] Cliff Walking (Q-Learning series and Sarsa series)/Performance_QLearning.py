...
if __name__ == "__main__":
    ...
    for env_index in range(num_test):        
        ...
        agents.append(QLearning(env_type, alpha, gamma, epsilon, seeds[env_index])) 
        ...
        for i, agent in enumerate(agents):
            ...
            if not dones[i]:
                if terminateds[i] or truncateds[i]:
                    ...
                    agent.update_Q_table(observations[i], actions[i], rewards[i], infos['final_observation'][i], batch_size=5)  
                else:
                    agent.update_Q_table(observations[i], actions[i], rewards[i], next_observations[i], batch_size=5)
    ...
    # 绘制 return 变化图
    np.save('data/QLearning_replay=5', envs_returns)
